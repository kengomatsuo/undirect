...
Undirect watches for those shapes and undoes them. The invisible layer goes before your press lands, so the click reaches what you aimed at. A link rewritten mid-click is put back, and you arrive where you meant to. A new window opens only when a site asks for one at a size, the way a sign-in does.
...
REQUIREMENTS
Undirect is a Safari extension. Switch it on in Safari, then press its toolbar button on each site you want guarded.
...
iOS and iPadOS
1. Launch Undirect, press Open Safari Extensions. Settings opens on Undirect under Safari Extensions. If it opens elsewhere, go to Apps, then Safari, then Extensions, then Undirect.
2. Turn on Undirect and set All Websites to Allow. The app notices within two seconds and shows its rules list.
...
The rules list is empty until the extension reports something, which happens the first time a guarded page attempts one of these tricks.
